...
int count_keys(tree_node_t *tree, key_t a, key_t b) {
  /* count_keys counts the keys x with a ≤ x ≤ b in the leaf nodes of the given search tree. The
  function should work with the given code for a height-balanced tree. */
  int count = 0;
  ...
  tmp = tree;
  while (tmp != NULL && (a <= tmp->key && b >= tmp->key)) {
    if (tmp->left == NULL) {
      break;
    }
    if (tmp->left->key <= b && tmp->left->key >= a) {
      count++;
      tmp = tmp->left;
    } else if (tmp->right->key <= b && tmp->right->key >= a) {
      count++;
      tmp = tmp->right;
    }
  }
